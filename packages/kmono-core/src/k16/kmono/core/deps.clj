...
(defn- relativize-path
  [base-dir path]
  (let [rel (str (fs/relativize base-dir path))]
    (if (= "" rel) "." rel)))


(defn- generate-extra-deps
  [base-dir packages]
  (reduce
    (fn [deps [fqn pkg]]
      (assoc deps fqn {:local/root (relativize-path base-dir (:absolute-path pkg))}))
    (sorted-map)
    packages))


(defn- scope-package-alias
  [package alias-name]
...
(defn- alter-path
  [base-dir package path]
  (let [resolved-path
        (fs/normalize
          (fs/file (:absolute-path package) path))]

    (relativize-path base-dir resolved-path)))


(defn- alter-paths
  [base-dir package paths]
  (mapv
    (partial alter-path base-dir package)
    paths))


(defn- alter-deps-paths
  [base-dir package deps]
  (reduce
    (fn [deps [fqn coord]]
      (if-let [path (:local/root coord)]
        (assoc deps fqn {:local/root (alter-path base-dir package path)})
...
(defn generate-sdeps-aliases
  "Generate an `-Sdeps` compatible map containing aliases generated from various
   ...
   Paths in the generated aliases are relativized to `base-dir`. When `base-dir`
   is not provided it defaults to `project-root`."
